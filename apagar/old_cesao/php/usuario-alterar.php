...
if($_SESSION["admin"] == 0){
    ...
    $usuarioId = $_COOKIE['pegaId'];

    extract($_POST);
    //verifica se o susuário chamado pelo Id realmente existe
    ...
    if($totalExiste->Id == 1){
        //se o email existir no cadastro de iutro usuário
        $verificaEmail = mysqli_query($con, "select COUNT(email) as email from usuario where email = '$email' and Id != '$usuarioId'");
        ...
        if($totalEmail->email == 0){
            ...
            if($senha == $senha2){
                ...
                if($_FILES['foto']['name'] !== ''){
                    ...
                    if($_FILES['foto']['error'] !== 0) {
                        ...
                        // O arquivo passou em todas as verificaушes, hora de tentar movЖ-lo para a pasta
                            ...
                            if(move_uploaded_file($_FILES['foto']['tmp_name'], $_UP['pasta'] . $nome_final)) {
                                ...
                                $sql = "update usuario set nome = '$nome', sobrenome = '$sobrenome', sexo = $sexo, cpf = '$cpf', rg = '$rg', telefone = '$telefone', email = '$email', ativo = '$ativo', foto = '$nome_final'";
                    ...
                    $sql = "update usuario set nome = '$nome', sobrenome = '$sobrenome', sexo = $sexo, cpf = '$cpf', rg = '$rg', telefone = '$telefone', email = '$email', ativo = '$ativo'";
